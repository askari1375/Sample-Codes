# -*- coding: utf-8 -*-
"""
Created on Tue Aug  3 10:47:58 2021
...
class DataPlotter:
    ...
    def check_hand_plot_time(self, duration):
        next_time = self.last_hand_plot_time + duration
        return next_time <= time.time()
    
    def check_error_plot_time(self, duration):
        next_time = self.last_error_plot_time + duration
        return next_time <= time.time()
    ...
    def first_plot(self):
        ...
        self.adjust_plot(axis_limits = self.find_axis_limits())
        for k in range(self.coordinates.shape[0]):
            one_hand = self.coordinates[k, :, :]
    ...
    def adjust_plot(self, axis_limits = None, title = None):
